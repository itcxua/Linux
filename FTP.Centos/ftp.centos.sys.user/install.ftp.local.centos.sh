...
# ADD пользов., файлы и каталоги для конфигурации. Добавим пользователя ftp в систему:
userdel -s /sbin/nologin ftpuser
...
# пересоздаем
cat /etc/pam.d/vsftpd > /etc/pam.d/vsftpd.$DATA.bac
